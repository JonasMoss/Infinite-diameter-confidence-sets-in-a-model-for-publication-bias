...
PB 1053, Blindern, NO-0316, Oslo, Norway\emph{}\\
\emph{jonasmgj@math.uio.no}}
\maketitle
\begin{abstract}
...
estimating and infering with this model is tough. Using a generalized
...
\end{abstract}
\keywords{meta-analysis \and confidence intervals \and file-drawer  problem \and publication bias \and selection models \and weigth function models}

\section{Introduction}

A meta-analysis is a statistical analysis that quantiatively combines
...
not account for it, as the parameter estimates will be incosistent
\citep{Carter2019-rw}. 
...
\end{eqnarray*}
Here $\sigma_{i}$ is the standard deviation of the $i$th study.
Following the convention in meta-analysis, we assume all $\sigma_{i}$s
to be known. The mean parameter $\theta$ is the population effect
size, $N(\theta_{0},\tau)$ is the effect size distribution, and $\tau$
is the heterogeneity parameter. The purpose of the effect size distribution
is to model the fact that most effect size estimates plugged into
a meta-analysis do not appear to measure the same phenomenon. By integrating
out $\theta_{i}$, we find the density of $x_{i}$,
\[
...
severely affected by publication bias, such as psychology, a non-negligble
...
on $\alpha$ and $\rho$ as
\begin{equation}
w(u;\rho,\alpha)=\sum_{k=1}^{K}\rho_{k}1_{(\alpha_{k-1},\alpha_{k}]}(u).\label{eq:step function}
\end{equation}
We call the selection model with a step function \emph{Hedges' publication
bias model. }Its density is
\begin{equation}
f(x_{i};\theta_{0},\sqrt{\tau^{2}+\sigma_{i}^{2}})\propto\sum_{k=1}^{K}\rho_{k}\phi(x_{i};\theta_{0},\sqrt{\tau^{2}+\sigma_{i}^{2}})1_{(\alpha_{k-1},\alpha_{k}]}(u_{i}).\label{eq:hedges model}
\end{equation}
Interpretating Hedges' publication bias model is easy. When the editor
receives a study with \emph{p}-value $u$, she finds the $j$ such
that $u\in(\alpha_{j-1},\alpha_{j}]$ and accepts with probability
$\rho_{j}$. Since $\rho_{1}=1$, she always accepts when $u\in[0,\alpha_{1}]$.
...
Hedges' publication bias models allows both for non-significant studies
...
funnel funnel plot of \citet{Egger1997-rd} is a graphical method,
while the trim-and-fill method of \citet{Duval2000-ct} is a non-parametric
...
space, and $\mathcal{\Pi}$ a partition of $\mathcal{P}$. Recall
that a partition of $\mathcal{P}$ is a collection of disjoint non-empty
subsets $\pi$ of $\mathcal{P}$ such that $\bigcup_{\pi\in\Pi}\pi=\mathcal{P}$.
...
\label{thm:Gleser--Hwang} Suppose there is a sequence $\{p_{i}\}$
of densities from $\mathcal{P}$ satisfying the following: 
\begin{enumerate}
\item There is a density $p^{\star}$ such that $p_{i}$ converges to $p^{\star}$
pointwise,
\item $\supp p\supseteq\supp p^{\star}$ for all $p$,
\item the size of $[p_{i}]$ goes to infinity, $\left\Vert [p_{i}]\right\Vert \to\infty$.
\end{enumerate}
...
of Theorem \ref{thm:Gleser--Hwang} for some suitable partition $\Pi$
...
\begin{example}[Fieller's problem]
\label{exa:Fieller's problem} Let $X,Y$ be an observation from
a bivariate normal $N([\mu_{1},\mu_{2}],I\sigma^{2})$, where $\sigma^{2}$
is known. We want to form a confidence set for ratio $\mu_{2}/\mu_{1}$.
The most famous confidence set is due to \citet{Fieller1940-lg}.
His confidence set can be finite, the whole real line, or the union
of two disjoint semi-infinite intervals \citep{Koschat1987-dk}, all
with positive probability.
...
set for $\mu_{2}/\mu_{1}$ must be infinitily long with positive probability.
...
\begin{example}[Bahadur--Savage]
\label{exa:Bahadur--Savage} \citet{bahadur1956nonexistence} studies
non-parametric testing of the mean, and concludes the mean cannot
be meaningfully tested. They are working with a family $\mathcal{P}$
of densities over $\mathbb{R}$ that covers all finite means, has
finite variances, and is closed under convex combinations. Similar
problems were considered by \citet{romano2004non} and \citet{Donoho1988-hg}.
...
This information is summarised in Table \ref{tab:Possible partitions}
for convenience. 
...
Using Lemma \pageref{lem:generalized shifted exponential} it is not
hard to show that the basic publication bias model (\ref{eq:selection for significance model})
...
Let $f^{1},f^{2},\ldots,f^{K}$ be a sequence of densities, $\pi=(\pi_{1},\pi_{2},\ldots,\pi_{K})$
...
above. Assume there is a sequence $p_{i}=\sum_{k\leq K}\pi_{i}^{k}f_{i}^{k}$
and a subset $K'$ such that 
\begin{enumerate}
\item For all $k\in K'$, there is a density $f^{k\star}$ such that $f_{i}^{k}$
converges to $f^{k\star}$ pointwise.
\item For all mixtures $p$, $\supp p\supseteq\supp f^{k\star}$ for some
$k\in K'$.
\item For all $k\in K'$, the size of $[f_{i}^{k}]$ goes to infinity, $\left\Vert [f_{i}^{k}]\right\Vert \to\infty$.
\item The density concentrates on the components indexed by $K'$, $\lim_{i\to\infty}\sum_{k\in K'}\pi_{i}^{k}=1$.
\end{enumerate}
Then every confidence set with level $\alpha>0$ has infinite diameter
with positive probability.
\end{lem}
...
\begin{proof}
We employ Theorem \ref{thm:Gleser--Hwang}. By (i) and (iv), $p_{i}$
converges pointwise to the density $\sum_{k\in K'}\pi_{i}^{k\star}f_{i}^{k\star}=p^{\star}$
. That $\supp p\supseteq p^{\star}$ follows from (ii) and (iv). Finally,
from the assumption that$\left\Vert [p]\right\Vert =\left\Vert [f_{k}]\right\Vert $,
we get that $\left\Vert [p]\right\Vert \to\infty$ too. 
\end{proof}
\begin{thm}
...
of Hedges' model can be made routine, as it is easy to find uncontroverisal
...
be intepreted as the aggregation of many small effects, the central
...
is a decent standard prior. Similarily, a half-normal is a reasonable
...
then $\int f_{n}d\mu\to\int fd\mu$.
\end{lem}

The proof of Theorem \ref{thm:Gleser--Hwang} follows the proof of
\citet[Theorem 1]{gleser1987nonexistence} closely. 
\begin{proof}[Proof of Theorem \ref{thm:Gleser--Hwang}]
\label{proof:Gleser--Hwang}We can assume without loss of generality
that $\left\Vert [p_{n}]\right\Vert \geq n$, as we can choose a subsequence
...
too. It follows that $P(D=\infty)>0$, and, since $P$ is aribtrary,
...
that $\sigma_{n}^{2}>0$. Recll the well-known formula for the normal
truncated to $[a,b]$,
\begin{eqnarray}
...
\[
c=\sum_{k=1}^{K}\rho_{k}[\Phi(c_{j-1};\theta_{0},(\tau^{2}+\sigma_{i}^{2})^{1/2})-\Phi(c_{j};\theta_{0},(\tau^{2}+\sigma_{i}^{2})^{1/2})].
\]
Then $\pi_{K}=\rho_{K}c^{-1}$ and \label{eq:pi_i formula}
\[
\pi_{k}=c^{-1}(\rho_{k}-\rho_{K})[\Phi(c_{k-1};\theta_{0},(\tau^{2}+\sigma_{i}^{2})^{1/2})-\Phi(c_{k};\theta_{0},(\tau^{2}+\sigma_{i}^{2})^{1/2})]
\]
